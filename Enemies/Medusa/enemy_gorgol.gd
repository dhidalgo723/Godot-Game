...
@export var vision_range : float = 140.0        # ← distancia a la que ve al jugador
@export var vision_height : float = 64.0        # ← tolerancia vertical de la vision
@export var attack_range : float = 40.0         # ← distancia a la que ataca
@export var attack_cooldown : float = 1.2       # ← segundos entre ataques
...
const GRAVITY = 1000
enum State { idle, walk, hit, dead, idle_wait, chase, attack }
...
const IDLE_WAIT_TIME : float = 1.0
...
func _physics_process(delta : float):
	...
	if current_state == State.idle_wait:
...
func _on_animation_finished():
	if current_state == State.attack:
		...
		if current_hits >= max_hits:
			...
			current_state = State.idle_wait
			idle_wait_timer = IDLE_WAIT_TIME
			can_walk = false
